...
function EditSocialMedia() {
    const {socialId,itemId} = useParams()
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
       if(res.status===200){
        axios.get(`${SERVER_URL}/admin/get-social-media/${socialId}/${itemId}`).then((res) => {
            if (res.status === 200) {
              setSocialMedia({
                name: res.data[0].name,
                postion: res.data[0].position,
  ...
  const handleSubmit = (e) => {
    ...
    const token = localStorage.getItem("token");
    axios
      .post(`${SERVER_URL}/admin/update-social-media-details/${socialId}/${itemId}`, formData, {
        headers: { "x-access-token": token },
      })
      .then((res) => {
        if (res.status === 200 || res.status === 201) {
          toast.success("SocialMedia update successfully");
          setSocialMedia({
            name: "",
