...
function AddPanchayath() {
  ...
  const handleAssemblyChange = (e) => {
    ...
    axios
      .get(
        `${SERVER_URL}/admin/districtV3?district=${district}&constituency=${Constituency}&assembly=${selectedAssembly}`,
        {
          // Use the updated district value
          headers: { "x-access-token": localStorage.getItem("token") },
        }
      ...
      .then((userResponse) => {
        if (userResponse.status === 200) {
          setPanchayathList(userResponse.data);
        }
      })
      .catch((err) => {
        console.log(err.response.data);
  ...
  const handleSubmit = (e) => {
    ...
    if (panchayathList?.includes(panchayath)) {
      toast.error("Panchayath Already Exist");
      return;
    }
    if (district == "") {
      toast.error("Select The District");
    ...
    axios
      .post(
        `${SERVER_URL}/admin/add-panchayath`,
        {
          name: panchayath,
          district: district,
      ...
      .then((res) => {
        if (res.status === 200 || res.status === 201) {
          setPanchayathList((prev) => [...prev, panchayath]);
  ...
  const handleDelete = (name) => {
    ...
    axios
      ...
      .then((res) => {
        if (res.status === 200) {
          setPanchayathList(
            panchayathList?.filter((product) => product !== name)
          );
        }
      })
      .catch((err) => {
            ...
            <div className="col-md-6 grid-margin stretch-card">
              <div className="card">
                <div className="card-body">
                  <h6 className="card-title">Add Panchayath</h6>
